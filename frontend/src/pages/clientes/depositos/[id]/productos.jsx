...
export default function ProductosDeposito() {
  ...
  // Encontrar el depósito (comparar como string para soportar UUIDs)
  const deposito = depositos.find((d) => String(d.id) === String(depositoId));
  ...
  const productos = Array.isArray(deposito.productos) ? deposito.productos : [];
...
function ProductoCard({
  ...
  return (
    <div
      className={`card hover:shadow-lg transition-shadow overflow-hidden group ${sinStock ? "opacity-75" : ""}`}
    >
      {/* Image */}
      <div className="relative -mx-6 -mt-6 mb-4 h-48 bg-gray-100 overflow-hidden">
        {producto.imagen ? (
          <img
            src={producto.imagen}
            alt={producto.nombre}
            className={`w-full h-full object-cover group-hover:scale-105 transition-transform duration-300 ${sinStock ? "grayscale" : ""}`}
          />
        ) : (
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between">
          ...
          <p
            className={`text-sm ${sinStock ? "text-red-500 font-medium" : "text-gray-500"}`}
          >
        ...
        <button
          ...
          disabled={sinStock}
          className={`w-full mt-2 ${sinStock ? "bg-gray-300 text-gray-500 cursor-not-allowed py-2 px-4 rounded-lg font-medium" : cantidadEnCarrito > 0 ? "btn-secondary" : "btn-primary"}`}
        >
          {sinStock
            ? "❌ Sin stock"
            : cantidadEnCarrito > 0
...
function ProductoListItem({
  ...
  return (
    <div
      className={`card hover:shadow-lg transition-shadow ${cantidadEnCarrito > 0 ? "ring-2 ring-primary/30" : ""} ${sinStock ? "opacity-75" : ""}`}
    >
        ...
        <div className="flex-1 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              onClick={() => onAgregarCarrito(producto)}
              disabled={sinStock}
              className={`whitespace-nowrap ${sinStock ? "bg-gray-300 text-gray-500 cursor-not-allowed py-2 px-4 rounded-lg font-medium" : cantidadEnCarrito > 0 ? "btn-secondary" : "btn-primary"}`}
            >
              {sinStock
                ? "❌ Sin stock"
                : cantidadEnCarrito > 0
